...
def make_tools(collector_database_url: str) -> list:
        ...
        try:
            with psycopg2.connect(collector_database_url) as conn:
                with conn.cursor() as cur:
                    ...
                    """, {"days": days})
                    rows = cur.fetchall()
            lines = ["prefix,flights,avg_alt_ft,min_alt_ft,max_alt_ft,closest_nm,avg_dist_nm"]
            for r in rows:
            ...
            with psycopg2.connect(collector_database_url) as conn:
                with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
                    ...
                    for row in repeats:
                        ...
                        visits = cur.fetchall()
                        callsign = next((v["callsign"] for v in visits if v["callsign"]), None)
                        result.append({
                            "hex": row["hex"],
                            "callsign": callsign,
                            ...
                            "visits": [
                                {
                                    ...
                                    "min_dist_nm": round(v["min_distance"], 2) if v["min_distance"] else None,
                                }
                                for v in visits
                            ],
                        })
                ...
                for k, v in d.items():
                    if hasattr(v, "isoformat"):
                        ...
                        try:
                            ...
                            pass
            return json.dumps({"strongest_signal": strongest, "weakest_signal": weakest})
        except Exception as exc:
    ...
    def get_callsign_history(icao_hex: str) -> str:
        ...
        try:
            ...
            lines = [f"hex:{icao_hex},total_sightings:{total},"
    ...
    def get_distance_percentiles(days: int = 7) -> str:
        ...
        try:
            ...
            return (f"p50_nm:{p50},p90_nm:{p90},p99_nm:{p99},"
